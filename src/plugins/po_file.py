...
def handle_open(dialog, event):
    with wx.FileDialog(
        dialog,
        "Open file",
        wildcard="(*.cpf)|*.cpf|(*.txt)|*.txt|(*.dat)|*.dat|(All *.*)|*.*",
            ...
            if active == "1":
                for i in range(3, 6):
                    dialog.grid.SetCellBackgroundColour(
                        line_id, i, wx.Colour(255, 255, 50)
                    )  # yellow
            else:
                for i in range(3, 6):
                    dialog.grid.SetCellBackgroundColour(
                        line_id, i, wx.Colour(240, 240, 180)
                    )  # light yellow
...
def save(dialog):
    with open(dialog.file_path, "w", newline="") as file:
        ...
        file.write(
            f"{'# Active':<10}{'Designator':<20}{'Footprint':<40}{'X[mm]':<15}{'Y[mm]':<15}{'Rotation[deg]':<15}\n"
        )
        ...
        for row in range(dialog.grid.GetNumberRows()):
            row_data = [
                dialog.grid.GetCellValue(row, col)
                for col in range(dialog.grid.GetNumberCols())
            ]
            formatted_row = ""
            ...
            for idx, value in enumerate(row_data):
                if idx == 0:
                    ...
                    if " " in value:
                        ...
                        dialog.log.AppendText(
                            f"Footprint name spaces converted to underscores:  {value}\n"
                        )
...
def handle_save_as(dialog, event):
    with wx.FileDialog(
        ...
        if not any(dialog.file_path.lower().endswith(ext) for ext in valid_extensions):
            dialog.file_path += valid_extensions[
                0
            ]  # Appending the first valid extension
